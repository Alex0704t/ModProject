...
void Get_Time_String(void* in, char* s, uint8_t view_mode) {
  ...
  if(view_mode == view_time)
    snprintf(s, 17, "%.2d:%.2d:%.2d", temp.hour, temp.min, temp.sec);
  else if(view_mode == view_date)
    snprintf(s, 17, "%.2d.%.2d.%.2d", temp.date, temp.month, temp.year);
  else
    snprintf(s, 17, "%.2d.%.2d.%.2d %.2d:%.2d:%.2d", \
             temp.date, temp.month, temp.year, temp.hour, temp.min, temp.sec);
}

uint32_t get_fattime (void) {
  rtc_time_s temp = Get_Time();
  return (uint32_t) (((temp.year + 20) << 25) | (temp.month << 21)  | \
      (temp.date << 16) | (temp.hour << 11) | (temp.min << 5) | temp.sec);
}
